...
//
// Формат файлов общий с Python-версией: обе читают одни и те же .env,
// chats.toml и data/.
package config
...
func findRoot() string {
	...
	// бинарник лежит где-то внутри установки (go/bin/tg.exe, bin/tg.exe) —
...
// PeerString — peer как строка, в том виде, как его печатает Python str().
...
func (s *Settings) PeersPath() string       { return filepath.Join(s.DataDir(), "go-peers.json") }
...
func load(requireCredentials bool) (*Settings, error) {
	...
	// своя сессия Go-версии: с Telethon-файлом (SQLite) она не пересекается
	session := os.Getenv("TG_GO_SESSION")
	if session == "" {
		session = "data/go-user.session.json"
	}
